...
class CombinedRolloutLoss(nn.Module):
    ...
    def __init__(self,
                 ...
                 use_perceptual_norm: bool = False,
                 phase: int = 1):
        super().__init__()
        ...
        self.use_perceptual_norm = use_perceptual_norm
        self.phase = phase
    ...
    def forward(self,
                ...
                stroke_num_weight_override: float = None) -> dict:
        ...
        # stroke_num (支持 curriculum override)
        sn = stroke_num_cost(pred_seq)
        comp['stroke_num'] = sn.detach().float().item()
        sn_weight = stroke_num_weight_override if stroke_num_weight_override is not None else self.stroke_num_weight
        total = total + sn_weight * sn

        # smoothness / angle（按原版 phase 2 才打开）
        if self.smoothness_weight > 0:
            sm = smoothness_cost(pred_seq)
            comp['smoothness'] = sm.detach().float().item()
            # 原版乘以 stroke_num_loss_weight；这里用动态 sn_weight
            total = total + self.smoothness_weight * sn_weight * sm
        if self.angle_weight > 0:
            ag = angle_cost(pred_seq)
            comp['angle'] = ag.detach().float().item()
            total = total + self.angle_weight * sn_weight * ag
